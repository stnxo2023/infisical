...
<Steps>
    ...
    <Step title="Configure user details and role">
        ...
        <Note>
            **Transfers ownership—you cannot rely on grants alone.** Grants on the database and schema cover access, but **each existing secret in the sync target schema stays owned by whoever created it** unless you intentionally move ownership. Infisical’s Snowflake user must run with a role that **owns every secret Infisical will manage** (`GRANT OWNERSHIP ON ALL SECRETS IN SCHEMA ...` plus `OWNERSHIP ON FUTURE SECRETS IN SCHEMA ...` as shown above). If anything in that schema is still owned elsewhere, syncing can fail when Infisical runs `CREATE OR REPLACE SECRET` or `DROP SECRET`.
        </Note>
    </Step>
