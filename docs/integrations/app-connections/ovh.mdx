...
<Steps>
  ...
  <Step title="Select your OKMS domain">
    Select the OKMS domain you want to use (or create a new one). Take note of the following values — you will paste them into Infisical in a later step:

    - **OKMS ID** — the UUID-style identifier shown on the OKMS summary page.
    - **REST API endpoint** — the base URL of the OKMS REST API (e.g. `https://ca-east-bhs.okms.ovh.net/api`). This is the **OKMS Domain** value in Infisical.

    ![OKMS Summary](/images/app-connections/ovh/okms-summary.png)
  </Step>
  <Step title="Generate an access certificate">
    Open the **Access certificate** tab and generate a new certificate. Download the two PEM files offered:
  ...
  <Tab title="Infisical UI">
    <Steps>
      ...
      <Step title="Fill out the OVH Cloud Connection form">
        ...
        - **OKMS Domain** — the OKMS REST API endpoint (e.g. `https://ca-east-bhs.okms.ovh.net/api`).
    ...
    curl    --request POST \
            ...
            --data '{
                ...
                "credentials": {
                    ...
                    "okmsDomain": "https://ca-east-bhs.okms.ovh.net/api",
