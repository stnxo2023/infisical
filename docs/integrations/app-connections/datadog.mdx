...
<Steps>
    ...
    <Step title="Create the Service Account">
        ...
        <Note>
            The Service Account's role determines which resources Infisical can access. Make sure it has access to the resources you want Infisical to manage. For secret rotation, it is required to have the `Datadog Admin Role`
        </Note>
    ...
    <Step title="Configure Application Key Scopes">
        ...
        you need to set `user_app_keys` under **API and Application Keys** and `service_account_writer`, `user_access_manage` and `user_access_read` under **Access Management**
